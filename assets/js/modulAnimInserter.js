...
function mib3Anim(content) {
    ...
    tl1.from(content.getElementById('cls3'), {
        x: 100,
    }, '+=0.01')

    tl.from(content.getElementsByClassName('cls-21'), {
        y: -400,
        ease: 'power3.out'
    }, '+=0.2').from(content.getElementsByClassName('cls-26'), {
        x: -400
    }, '-=0.2').from(content.getElementsByClassName('cls-27'), {
        x: -400,
        stagger: 0.2
    }, '+=0.3').from(content.getElementsByClassName('cls-22'), {
        y: 100
    }, '-=0.2').from(content.getElementsByClassName('cls-23'), {
        x: -400,
        stagger: 0.2
    }, '-=0.2');
    
    
    tl.from(content.getElementsByClassName('cls-28'), {
        duration: 0.5,
        stagger: 0.2,
        x: "-100vw",
        ease: "power3.out",
    ...
    let cursor = gsap.to(content.getElementById('cursor'), {
        opacity: 0,
        ease: 'power2.inOut',
        repeat: -1
    });
    ...
    words.forEach(word => {
        ...
        tl.to(content.getElementById('text'), {
            duration: 1,
            text: word
        })
        masterTL.add(tl);
